```
//@version=6
indicator('乘舟战神指标',"乘舟战神指标", overlay=true,max_labels_count = 500, max_lines_count = 500, max_boxes_count = 500)
...
// 参数
emaShowInput = input(true,title="显示EMA均线",group=EMA_GROUP)
emaShortLineInput = input.int(21,title='短期均线',group=EMA_GROUP)
emaLongLineInput = input.int(55,title='长期均线',group=EMA_GROUP)
...
rsiLengthInput = input(14,title='RSI长度',group=RSI_GROUP)
rsiCandlShowInput = input(true,title='显示RSI蜡烛',group=RSI_GROUP)
rsiUpColor   = input.color(color.new(color.purple, 0), "RSI蜡烛超买",group=RSI_GROUP)
rsiDownColor = input.color(color.new(color.yellow, 0), "RSI蜡烛超卖",group=RSI_GROUP)
rsiOverSoldInput = input.int(30,title='RSI超卖阈值',group=RSI_GROUP) 
rsiOverBoughtInput = input.int(70,title='RSI超买阈值',group=RSI_GROUP)
rsiDivergenceShowInput = input(true,title='是否显示RSI背离',group=RSI_GROUP)
rsiDivergenceLengthInput = input.int(25,title='RSI背离的长度',group=RSI_GROUP, maxval = 500)
...
rsiStatus = rsiIsUp ? "超买" : rsiIsDown ? "超卖" : "正常"
rsiSymbol = rsiStatus == "超买" ? "🔴" : rsiStatus == "超卖" ? "🟢" : "⚪️"

// RSI 超买/超卖 蜡烛
// 只展示 4小时级别 & 日线级别的超卖/超买,其他时间级别的不重要....
...
adx_length = input.int(10, "ADX长度",group = ADX_GROUP)
...
volumeLengthInput = input.int(20,title='平均成交量长度',group=VOLUME_GROUP)
mildVolumeInput = input.float(1.2,title='普通爆量倍数',group=VOLUME_GROUP)
strongVolumeInput = input.float(1.5,title='中级爆量倍数',group=VOLUME_GROUP)
extremeVolumeInput = input.float(2,title='高级爆量倍数',group=VOLUME_GROUP)
buyShowInput = input(true,title='显示buy信号',group=VOLUME_GROUP)
buyBreakInput = input.int(20,title='最高/最低价格范围',group=VOLUME_GROUP)
buyWatchInput = input.int(5,title='买入监控的K线总数',group=VOLUME_GROUP)
sellWatchInput = input.int(3,title='卖出监控的K线总数',group=VOLUME_GROUP)
buyVolumeInput = input.float(1,title='跟随成交量倍数',group=VOLUME_GROUP)
buyRequiredInput = input.int(2,title='买入跟随K线总数',group=VOLUME_GROUP)
sellRequiredInput = input.int(1,title='卖出跟随K线总数',group=VOLUME_GROUP)
springShowInput = input(true,title='显示多空信号',group=VOLUME_GROUP)
springLengthInput = input.int(20,title='最高/最低价格范围',group=VOLUME_GROUP)
...
// 条件 A: 当前 K 线低点就是最低点
// 条件 B: 前一根 K 收阴线低点是最低点 (意味着当前这根是在低位震荡或微幅抬升的放量阳线)
is_near_anchor_low = (low == bull_anchor_low) or (low[1] == bull_anchor_low and close[1] < open[1])
// 爆量K的条件: 成交量1.2倍,最低点或者次低点,阳线
is_main_break = volume >= vol_ma * mildVolumeInput and is_near_anchor_low and close > open
...
if buyShowInput and is_lower_than_4h
    ...
    // 爆量已经出现并且启动了监控了
        ...
        if volume > vol_ma * buyVolumeInput and close > open // 必须保证是阳线跟随
            follow_vol_count += 1
        // 若5根内出现至少2根放量，则确认成功
...
prev_low = ta.lowest(low, springLengthInput)[1] // 50根K线的最低点,不包含当前K
// 前一根阴线判断
...
bull_upper_wick = curr_is_bull ? high - close : 0
// 不允许上影线大于实体的1.2倍,可以一样长,但是不能过长
bull_upper_wick_ok = curr_is_bull and bull_upper_wick <= curr_bull_body * 1.2
spring_low_signal = prev_is_bear and curr_is_bull and low[1] == prev_low and close > low[1] and bull_strong_enough and bull_upper_wick_ok
// 量能要求
stars_low =
     volume[1] >= vol_ma * 1.5 and volume >= vol_ma * 1.5 ? "⭐⭐⭐" :
     volume[1] >= vol_ma * 1.2 and volume >= vol_ma * 1.2 ? "⭐⭐"  :
     volume[1] >= vol_ma * 1   and volume >= vol_ma * 1   ? "⭐"   :
     ""
// 一星
plotshape(
    springShowInput and is_1h_or_higher and  spring_low_signal and stars_low == "⭐",
    style = shape.labelup,
...
// 同时如果是上影线也可以满足条件
bear_upper_wick = curr_is_bear ? high - close : 0
// 如果实体很小,但是上影线非常的大,依旧可以满足空的逻辑
bear_upper_wick_ok = curr_is_bear and bear_upper_wick > curr_bear_body * 1.2
// 信号
spring_high_signal = prev_is_bull and curr_is_bear and high[1] == prev_high and close < high[1] and (bear_strong_enough or bear_upper_wick_ok)
...
// 一星（空）
plotshape(
    springShowInput and is_1h_or_higher and spring_high_signal and stars_high == "⭐",
...
tableShowInput = input(true,title='是否展示表格',group=TABLE_GROUP)
if tableShowInput
    ...
    string cont_vol_str = has_following_vol ? "🔥🔥🔥" : ""
